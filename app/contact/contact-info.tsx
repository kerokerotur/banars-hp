...
export default function ContactInfo({ onLocationClick, onPhoneClick, onEmailClick }: ContactInfoProps) {
            ...
            {item.content.map((line, index) => (
              <p
                ...
                className={`text-gray-300 text-sm ${
                  item.title === "Phone" || item.title === "Email" || item.title === "Location"
                    ? "cursor-pointer hover:text-pink-400 transition-colors"
                    : ""
                }`}
                onClick={() => {
                  if (item.title === "Phone" || item.title === "Email" || item.title === "Location") {
